...
The plugin works by saving edited or new articles/posts directly to the file system. It then uses Docpad's inbuilt system for regenerating a page when the corresponding source file is modified. To give finer control over the regeneration process the plugin will manually trigger the regeneration of the document just updated. This works best when Docpad is run with the `docpad server` command rather than `docpad run`, otherwise the regeneration process will run twice. If you want docpad to handle all regeneration on its own, then set the config option `handleRegeneration: false`.
